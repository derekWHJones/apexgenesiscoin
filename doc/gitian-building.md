...
*Setup instructions for a Gitian build of ApexGenesiscoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ApexGenesiscoin
...
- [Building ApexGenesiscoin Core](#building-apexgenesiscoin-core)
...
Clone the git repositories for apexgenesiscoin and Gitian.
...
in the apexgenesiscoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building ApexGenesiscoin Core
----------------

To build ApexGenesiscoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the apexgenesiscoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/apexgenesiscoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/apexgenesiscoin-project/apexgenesiscoin
...
For example:
```bash
URL=https://github.com/thrasher-/apexgenesiscoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit apexgenesiscoin=${COMMIT} --url apexgenesiscoin=${URL} ../apexgenesiscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit apexgenesiscoin=${COMMIT} --url apexgenesiscoin=${URL} ../apexgenesiscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit apexgenesiscoin=${COMMIT} --url apexgenesiscoin=${URL} ../apexgenesiscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the apexgenesiscoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../apexgenesiscoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/apexgenesiscoin
SIGPATH=/some/root/path/apexgenesiscoin-detached-sigs

./bin/gbuild --url apexgenesiscoin=${BTCPATH},signature=${SIGPATH} ../apexgenesiscoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/apexgenesiscoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/apexgenesiscoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/apexgenesiscoin-osx-build.assert
```
...
[apexgenesiscoin-project/gitian.sigs.ltc](https://github.com/apexgenesiscoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
